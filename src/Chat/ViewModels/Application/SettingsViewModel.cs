    ...
    public async Task<bool> SavePasswordAsync()
    {
        ...
        return await RunCommandAsync(() => PasswordIsChanging, async () =>
        {
            ...
            var result = await Dna.WebRequests.PostAsync<ApiResponse>(
                ...
                {
                    CurrentPassword = SecureStringHelpers.Unsecure((SecureString?)Password.CurrentPassword),
                    NewPassword = SecureStringHelpers.Unsecure((SecureString?)Password.NewPassword)
                },
                // Pass in user Token
